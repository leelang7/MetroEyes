...
- test_eda_viz_hourly.py: is_urban/to_long/URBAN 9개 (cycle 530, mock 격리)
- 회귀 가드 673 → 692 (+19)


## [cycle 528] 2026-05-11
### Added
- test_llm_assistant_js.py: classifyIncident/CANNED_DB/다국어/urgency 11개
- 회귀 가드 662 → 692 (+11)
...
- test_carload_v2_model.py: LINE_META/synth_target/build_dataset 9개
- test_co2_correlation.py: correlate_pollutant/hourly_pattern 8개
- 회귀 가드 589 → 616 (+27)
...
### Added
- test_loaders_extract_rows.py: extract_rows 파싱 단위 가드 7개 (cycle 506)
...
### 신규 자료 (8 종)
- `frontend/onepager.html` (cycle 372/377/570) — A4 1-Pager 4언어
...
## v6.11 — D-2 회귀 가드 764건 + mobile_app Flutter + 시민신고 오프라인큐 + admin 시민신고LIVE + op/admin citizen_report + 8-Moat + i18n4언어 (2026-05-11 cycle 469-534)

### v6.11 cycle 570-496
- cycle 570: op/admin citizen_report 핸들러(applyCitizenReport) + 341→342 가드 전체 동기화
- cycle 496: passenger_app FAB 상태 머신 (스피너/_setReportBtnState/_updateQueueBadge/오프라인배지) + 342→570 가드 동기화
...
- cycle 570: SLIDES 시민신고·10공공API·ESG 가드 3건 추가
...
## v6.8 — D-4 회귀 가드 273건 + 30p 상세기획서 deck + citydata 통합 + 3D OpenFreeMap + dday.ps1/sh/Makefile + link checker + ad env + FORM_DATA + cycle 374 회복 (2026-05-08~09 cycle 391-570)

### v6.8 cycle 414-570 (3rd platform parity + install + Quick Start + body drift + 3D map + citydata fetch + 30p deck)
...
- cycle 570: 📑 **30 슬라이드 상세기획서 HTML deck** (사용자 직접 지시 — PPT 템플릿 → HTML 30p)
  ...
  - canonical KPI 7종 (1,393억/570×/473.4M/157M/708×/CI [1,064~1,808]) 모두 포함 — drift 차단
...
- cycle 400: 🎉 마일스톤 — 자동 모드 400 사이클 완주 + onepager 197/570 → 233/400
...
- cycle 389: onepager stale 146/377 → 197/570 갱신
...
- **cycle 369** 5분 발표 영상 4언어 narration 스크립트
  ...
  - cycle 356/358/570 신규 KPI 모두 4언어 노출
